...
class Road: SKSpriteNode {
    ...
    let road = SKTexture(imageNamed: "PinkRoad")
    ...
    func beginAnimation() {
        ...
        if isAnimating == false {
            ...
            let moveroadRightToLeftAction = SKAction.moveByX(-road.size().width, y: 0, duration: NSTimeInterval(speed * road.size().width))
            let resetRoadLeftToRightAction = SKAction.moveByX(road.size().width, y: 0, duration: 0.0)
            let movingroadActionSequence = SKAction.sequence([moveroadRightToLeftAction, resetRoadLeftToRightAction])
            
            runAction(SKAction.repeatActionForever(movingroadActionSequence))
        }
    }
