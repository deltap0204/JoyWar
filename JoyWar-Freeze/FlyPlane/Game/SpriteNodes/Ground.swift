...
class Ground: SKSpriteNode {
    ...
    let ground = SKTexture(image: #imageLiteral(resourceName: "Ground"))
    ...
    func beginAnimation() {
        ...
        if isAnimating == false {
            ...
            let moveGroundRightToLeftAction = SKAction.moveBy(x: -ground.size().width, y: 0, duration: TimeInterval(speed * ground.size().width))
            let resetGroundLeftToRightAction = SKAction.moveBy(x: ground.size().width, y: 0, duration: 0.0)
            let movingGroundActionSequence = SKAction.sequence([moveGroundRightToLeftAction, resetGroundLeftToRightAction])
            
            run(SKAction.repeatForever(movingGroundActionSequence))
        }
    }
