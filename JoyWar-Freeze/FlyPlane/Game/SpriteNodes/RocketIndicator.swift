...
class RocketIndicator: Button {
    ...
    init() {
        let texture = SKTexture(image: Plane.currentRocketType.getIndicatorImage())
...
// MARK: PlaneRocketGunDelegate

extension RocketIndicator: PlaneRocketGunDelegate {
    func updateRockets(count: Int) {
        updateCounter(with: count)
    }
    func startReloadCountdown(duration: Double) {
        timer.isHidden = false
        timer.countdown(time: 1.0) { [weak self] () -> Void in
