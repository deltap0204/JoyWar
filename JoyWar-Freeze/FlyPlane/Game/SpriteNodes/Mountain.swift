...
class Mountain: SKSpriteNode {
    ...
    let mountain = SKTexture(image: #imageLiteral(resourceName: "Mountain"))
    ...
    func beginAnimation() {
        ...
        if isAnimating == false {
            ...
            let moveMountainRightToLeftAction = SKAction.moveBy(x: -mountain.size().width, y: 0, duration: TimeInterval(speed * mountain.size().width))
            let resetMountainLeftToRightAction = SKAction.moveBy(x: mountain.size().width, y: 0, duration: 0.0)
            let movingMountainActionSequence = SKAction.sequence([moveMountainRightToLeftAction, resetMountainLeftToRightAction])
            
            run(SKAction.repeatForever(movingMountainActionSequence))
        }
    }
